...
const API_URL = "https://proximal-maddening-stock.glitch.me";
...
const modaloverlayCloseButton = document.querySelector(
  ".modal-overlay_close-button"
);
...
orderMessageCloseButton.textContent = "Закрити";
...
const fetchProductByCategory = async (category) => {
  try {
    ...
    console.error(`Помилка запиту товарів: ${error}`);
  }
};
...
const fetchCartItems = async (ids) => {
  try {
    ...
    console.error(`Помилка запиту товарів для кошику: ${error}`);
    return [];
  }
};
...
cartButton.addEventListener("click", async () => {
  ...
  if (!ids.length) {
    cartItemsList.textContent = "";
    const listItem = document.createElement("li");
    listItem.textContent = "Кошик порожній";
    cartItemsList.append(listItem);
    return;
  }
...
const submitOrder = async (e) => {
  ...
  try {
    ...
    orderMessageText.textContent = `Ваше замовлення оформлено, номер ${orderId}. Ви можете забрати його завтра після  12:00`;
    document.body.append(orderMessageElement);

    modalOverlay.style.display = "none";
    updateCartCount();
  } catch (error) {
    console.error(`Помилка оформлення заказу: ${error}`);
  }
};
